...
def recommend_movies(user_id, n=10, k=300, alpha=0.9):
    ...
    rating_counts = train["movie_id"].value_counts()

    unseen_movies = unseen_movies[
        unseen_movies["movie_id"].map(rating_counts).fillna(0) >= 20
    ]
    ...
    for _, movie in unseen_movies.iterrows():
        ...
        hybrid_score = predict_hybrid_rating(
            user_id=user_id,
            movie_id=movie_id,
            k=k,
            alpha=alpha
